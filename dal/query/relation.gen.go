...
func newRelation(db *gorm.DB, opts ...gen.DOOption) relation {
	...
	_relation.ID = field.NewInt32(tableName, "id")
	_relation.UserID = field.NewInt32(tableName, "user_id")
	_relation.ToUserID = field.NewInt32(tableName, "to_user_id")
	_relation.CreateAt = field.NewTime(tableName, "create_at")
	...
	_relation.FollowUser = relationBelongsToFollowUser{
		db: db.Session(&gorm.Session{}),

		RelationField: field.NewRelation("FollowUser", "model.User"),
	}
...
type relation struct {
	...
	ALL      field.Asterisk
	ID       field.Int32
	UserID   field.Int32 // 关注者id
	ToUserID field.Int32 // 被关注者id
	CreateAt field.Time
	User     relationBelongsToUser

	FollowUser relationBelongsToFollowUser
...
func (r *relation) updateTableName(table string) *relation {
	...
	r.ToUserID = field.NewInt32(table, "to_user_id")
...
type relationBelongsToFollowUser struct {
	db *gorm.DB

	field.RelationField
}

func (a relationBelongsToFollowUser) Where(conds ...field.Expr) *relationBelongsToFollowUser {
	if len(conds) == 0 {
		return &a
	}
...
func (a relationBelongsToFollowUser) WithContext(ctx context.Context) *relationBelongsToFollowUser {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a relationBelongsToFollowUser) Model(m *model.Relation) *relationBelongsToFollowUserTx {
	return &relationBelongsToFollowUserTx{a.db.Model(m).Association(a.Name())}
}

type relationBelongsToFollowUserTx struct{ tx *gorm.Association }

func (a relationBelongsToFollowUserTx) Find() (result *model.User, err error) {
	return result, a.tx.Find(&result)
}

func (a relationBelongsToFollowUserTx) Append(values ...*model.User) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a relationBelongsToFollowUserTx) Replace(values ...*model.User) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a relationBelongsToFollowUserTx) Delete(values ...*model.User) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a relationBelongsToFollowUserTx) Clear() error {
	return a.tx.Clear()
}

func (a relationBelongsToFollowUserTx) Count() int64 {
	return a.tx.Count()
}
